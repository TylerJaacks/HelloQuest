...
int render_gles_scene(XrCompositionLayerProjectionView &layerView, render_target_t &rTarget, XrPosef &stagePose, XrTime elapsed_us, uint32_t viewID) {
    ...
    XrMatrix4x4f_CreateTranslationRotationScale(&matC, &vewPose.position, &vewPose.orientation,
                                                &scale);
    XrMatrix4x4f_InvertRigidBody(&matV, &matC);

    /* Stage Space Matrix */
    XrMatrix4x4f_CreateTranslationRotationScale(&matM, &stagePose.position, &stagePose.orientation,
                                                &scale);
    ...
    /* Rendering */
    auto *matStage = reinterpret_cast<float *>(&matPVM);
...

    {
        ...
        sprintf(strBuffer, "VIEWPOS(%6.4f, %6.4f, %6.4f)", pos.x, pos.y, pos.z);
        draw_dbgstr(strBuffer, x, y);
        y += 22;

        sprintf(strBuffer, "VIEWROT(%6.4f, %6.4f, %6.4f, %6.4f)", rot.x, rot.y, rot.z, rot.w);
        draw_dbgstr(strBuffer, x, y);
        y += 22;

        sprintf(strBuffer, "VIEWFOV(%6.4f, %6.4f, %6.4f, %6.4f)",
